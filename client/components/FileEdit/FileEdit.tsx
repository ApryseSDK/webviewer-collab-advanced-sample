    ...
    <Modal onExit={onExit} loading={loading}>
      <Form onSubmit={onSubmit}>
        ...
        <FormField htmlFor='' label='Is Public?'>
          <CheckBox id='is-public' name='isPublic' checked={isPublic} onChange={(event) => setIsPublic(event.target.checked)} />
        </FormField>

        <Button type='submit' label='Save changes' primary margin={{ top: 'medium' }} style={{width: '100%'}} />
      </Form>
    </Modal>
  )
}